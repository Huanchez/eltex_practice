                                 ...
                                 const char *prompt, const char *old_base, size_t old_cnt)
{
    ...
    if (old_base && old_cnt) {
        ...
        puts("(Enter — оставить, '-' — удалить, новый ввод — заменить)");
...
// Приблизительный log2(n)
static size_t ilog2_floor(size_t n) {
    size_t r = 0;
    while (n >= 2) { n /= 2; r++; }
    return r;
}
...
int main(void){
    ...
    for(;;){
        ...
        if(choice==1){
            ...
            Contact *old = cb_find_by_id(&cb, id);
            if(!old){ puts("Контакт не найден."); continue; }

            puts("Текущая карточка:");
            cb_print_one(old);

            Contact patch;
            build_contact(&patch, old);

            if(!cb_edit(&cb, id, &patch)) puts("Ошибка редактирования.");
            else puts("Изменено.");

        } else if(choice==3){
            unsigned int id = read_uint("ID для удаления: ");
            if(!cb_remove(&cb,id)) puts("Не найден/ошибка удаления.");
            else puts("Удалено.");
            ...
            size_t h_before = cb_height(&cb);
            cb_balance(&cb);
            size_t h_after = cb_height(&cb);
            printf("Балансировка выполнена. Высота: %zu -> %zu\n", h_before, h_after);

        } else if(choice==6){
            size_t h = cb_height(&cb);
            size_t n = cb.count;
            size_t approx_log2 = (n == 0) ? 0 : ilog2_floor(n) + 1;
            printf("Контактов: %zu\n", n);
            printf("Высота дерева: %zu\n", h);
            printf("Ожидаемая высота у сбалансированного дерева ~ %zu\n", approx_log2);
