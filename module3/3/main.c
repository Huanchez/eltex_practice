...
int main(void){
    ...
    const char *db_path = "contacts.db";
    if (!cb_load(&cb, db_path)) {
        puts("Не удалось загрузить контакты из файла. Начинаю с пустой книги.\n");
    } else {
        printf("Загрузка завершена. Контактов в базе: %zu\n\n", cb.count);
    }
    ...
    for(;;){
        ...
        if(choice==1){
            ...
            Contact patch; build_contact(&patch, old);
    ...
    if (!cb_save(&cb, db_path)) {
        puts("Ошибка сохранения контактов в файл!");
    } else {
        puts("Контакты сохранены в файл");
    }
